<template lang="pug">
  .champion-deck-group.px-3(:style="{'borderColor': borderColor}")
    .class-title
      ...
      span.tooltip-container.synergy-description.mx-1(v-for="(item, index) in groupDescription.synergy" :key="`${groupDescription.id}${index}`")
        v-tooltip(top)
          template(v-slot:activator="{ on }")
            span(v-on='on') ({{ item.require }})
          .tooltip-type {{ item.bonus }}
